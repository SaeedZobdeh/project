...
// Hide the sidebar on close button click
closeSidebarButton.addEventListener("click", () => {
    if (sidebar.classList.contains("show")) {
        sidebar.classList.remove("show");
        sidebar.classList.add("hide");
    }
    menuToggle.classList.remove("is-active"); // Ensure the menu toggle button is not active
});
...
const tagCloseBtns = document.querySelectorAll('.tag-close-btn')
...
const setPriority = (value) => {
    ...
    tagItems.forEach(item => {
        const closeBtn = item.querySelector('.tag-close-btn')
        if (selectedPriority !== item.getAttribute('id')) {
...
tagCloseBtns.forEach(item => {
    item.addEventListener('click', (e) => {
        e.stopPropagation();
        document.querySelectorAll('.tag-close-btn').forEach(item => item.classList.add('hidden'));
